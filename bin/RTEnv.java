...
*************************************************************************** */
   ...
   {	int[]Mem;
      ...
      void SetFrom(int A){Mem[From]=A;}
		int GetFrom(){return Mem[From];}
      void Init(int sf,int pc){Mem[SF]=sf;Mem[DF]=Cur;Mem[PC]=pc;}
      int GetSF(){return Mem[SF];}
      int GetDF(){return Mem[DF];}
      int GetPF(){return Mem[PF];}
		int GetBlock(){return Mem[PC];}
      void SetPF(int r){Mem[PF]=r;}
...
*************************************************************************** */
	...
	{	int Low,High;
		int[] Contents;
		Array(int L,int H){Low=L;High=H;Contents=new int[H-L+1];}
	}
/* ***************************************************************************
	The constructor
...
*************************************************************************** */
   RTEnv(IStruct IO){Stack[0]=new AR(0);Stkp=1;InPut=IO;StdInit();}
/* ********************************************
	Setup environment.
The Operator precedence object and the interpreter object is registered
in the environment.
******************************************** */
	public void Init(OpPrec P,Ipass2 Q){TheOpPrec=P;TheInterpreter=Q;}
/* ********************************************
	Enter a block.
...
******************************************** */
   ...
   {  Template Tm;
      Tm=Templates[n];
      if(Tm==null){Templates[n]=new Template(n,sz);}
      else {}
   }
/* ********************************************
	Return from a block.
...
******************************************** */
   ...
   {  int s,sf,l; AR P;
      ...
      l=lev;
      sf=Cur;
      while(l>0){sf=Stack[sf].GetSF(); l=l-1;}
      P.Init(sf,n);
   }
/* ********************************************
	Deallocate an AR.
...
******************************************** */
   void DeAllocate(){Stkp=Par; Par=Stack[Par].GetPF();}
/* ********************************************
	Execute a block.
...
******************************************** */
   ...
   {  int Val; int ty1,ty2; float F;
		ty1=A1.a4;
		ty2=A2.a4;
      Val=GetVal(A2);
		if(ty1==ty2){}
		else
		if(ty2==C_Integer){F=(float)Val;Val=Float.floatToIntBits(F);}
		else
		if(ty2==C_Real){F=Float.intBitsToFloat(Val);Val=(int)F;}
		else{}
      SetVal(Val,A1);
   }
/* ********************************************
...
******************************************** */
   ...
   {  int V1,V2,V=0; float Vr1,Vr2,Vr=0;
		int ty1,ty2,ty3,ty;
		ty1=A1.a4;
		ty2=A2.a4;
		ty3=A3.a4;
      ...
      {	ty=C_Integer;
      	...
      	{case 4:	V=V1+V2; break;
       	...
       	case 7:	Vr=(float)V1/(float)V2; ty=C_Real; break;
       	default:	V=0; break;
      	}
		}
		else
		{	if(ty1==C_Integer){Vr1=(float)V1;}else{Vr1=Float.intBitsToFloat(V1);}
			if(ty2==C_Integer){Vr2=(float)V2;}else{Vr2=Float.intBitsToFloat(V2);}
			ty=C_Real;
			switch(op)
      	{case 4:	Vr=Vr1+Vr2; break;
       	case 5:	Vr=Vr1-Vr2; break;
       	case 6:	Vr=Vr1*Vr2; break;
       	case 7:	Vr=Vr1/Vr2; break;
       	default:	Vr=0; break;
      	}
		}
		if(ty==C_Integer)
		{	if(ty3==C_Integer){}
			else{Vr=(float)V; V=Float.floatToIntBits(Vr);}
		}
		else
		{	if(ty3==C_Integer){V=(int)Vr;}
			else{V=Float.floatToIntBits(Vr);}
		}
      SetVal(V,A3);
   }
/* ********************************************
...
******************************************** */
   ...
   {  int V1,V2; float Vr1,Vr2;
      ...
      V2=GetVal(A2);
		if(ty1==C_Integer&&ty2==C_Integer)
      {	switch(op)
      	{case 12:	V=V1>=V2; break;
       	case 13:	V=V1>V2; break;
       	case 14:	V=V1<=V2; break;
       	case 15:	V=V1>V2; break;
       	case 16:	V=V1==V2; break;
       	case 17:	V=V1!=V2; break;
		...
		{	if(ty1==C_Integer){Vr1=(float)V1;}else{Vr1=Float.intBitsToFloat(V1);}
			if(ty2==C_Integer){Vr2=(float)V2;}else{Vr2=Float.intBitsToFloat(V2);}
			switch(op)
      	{case 12:V=Vr1>=Vr2; break;
       	case 13:	V=Vr1>Vr2; break;
       	case 14:	V=Vr1<=Vr2; break;
       	case 15:	V=Vr1>Vr2; break;
       	case 16:	V=Vr1==Vr2; break;
       	case 17:	V=Vr1!=Vr2; break;
       	default:	V=false; break;
      	}
		}
      Cond=V;
   }
   boolean Condition(){return Cond;}
/* ********************************************
	Array handling

******************************************** */
	public void ALim(int I,Adr A1,Adr A2){Lw=GetVal(A1); Hg=GetVal(A2);}
	...
	{	int I,L,H,Ri; Array A;
		I=GetVal(A1);
		A=ArrayArea[I];
		L=A.Low;
		H=A.High;
		if(Ind>=L&&Ind<=H){Ri=Ind-L;}else{Ri=0;}
		Ri=A_Adel*I+Ri;
...
******************************************** */
   ...
   {  int M,N,V=0,ar,rel,typ;
      M=A.a1; rel=A.a3; typ=A.a4; Array TA;
      switch(M)
      {case 0: // in memory (Activation Record)
               N=A.a2;ar=Cur;
               if(N<0) {V=Stack[Par].GetVal(rel);}
               else
               {  while(N>0){ar=Stack[ar].GetSF();N=N-1;}
       ...
       case 5: // "register"
               V=0; 
               break;
       default:V=0;     // should not be used here
               break;
      }
      return V;
   }
   ...
   {  int M,rel,N,ar,typ; Array TA; int I;
      M=A.a1; rel=A.a3; typ=A.a4;
      switch(M)
      {case 0: // in memory (Activation Record)
               N=A.a2;ar=Cur;
               if(N<0) {Stack[Par].SetVal(rel,V);}
               else
               {  while(N>0){ar=Stack[ar].GetSF();N=N-1;}
...
******************************************** */
   infile StdFile;
   int IV; float RV; String TV;
   int IP; float RP; String TeP;
   public void TransfStdPar(int par){IP=par;}
   public void TransfStdPar(float par){RP=par;}
   public void TransfStdPar(String par){TeP=par;}
   public void TransfStdPar(Adr par)
	{	IV=GetVal(par);
		if(par.a4==C_Integer){}
		else
		if(par.a4==C_Real){RV=Float.intBitsToFloat(IV);}
		else
		if(par.a4==C_Text){TV=Literals[IV];}
		else{}
	}
   public void SetIV(Adr A){SetVal(IV,A);}
   public float GetRV(){return RV;}
   public String GetTV(){return TV;}
   
   public void ExecLib(int f)
   {  switch(f)
      {case 1:   ReadI();  break;   //readint
       case 2:   ReadR();  break;   //readreal
       case 3:   PrintI(); break;   //writeint
       case 4:   PrintR(); break;   //writereal
       case 5:   PrintT(); break;   //writetext
       case 6:   PrintL(); break;   //writeline
       case 7:   ReadT();  break;   //readtext
		 case 8:   	break;
		 case 9:   	break;
       default:  break;
      }
   ...
   private void StdInit(){StdFile=new infile(); StdFile.Open();}
   private void PrintI(){System.out.print(" "+IV);}
   private void PrintR(){System.out.print(" "+RV);}
   private void PrintT(){System.out.print(TV);}
   private void PrintL(){System.out.println(" ");}
   private void ReadI(){IV=StdFile.ReadInt();}
   private void ReadR()
   ...
   private void Print(String S, int V){System.out.println("Print "+S+" "+V);}
   ...
   {  AR P;
      ...
      {  P=Stack[i];
         if(P!=null){System.out.println("Dump of AR:"+i);P.Dump();}
      }
