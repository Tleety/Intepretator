...
****************************************************************** */	
	...
	{	Token T;
		...
		{	CreateBlock();
			...
			while (T.Decl())
			{	T=Deklaration(T);
				if (T.Semi()){T=GetToken();} else {Error(E_Sem,0);}
			}
			while (!(T.End()||T.Eof())) 
			{T=Sats(T);}
			CloseBlock(T);
...
****************************************************************** */
	...
	{	if (T.Func()) {T=Funktion(C_Void,0);}
		...
		{	int Q,Sid;
			Q=0;
			T=GetToken();
			if(!T.Lpar()) {Error(E_Lpar,0);}
			else T=GetToken();
			if(T.UserId()) Q=T.GetId();
			else Error(E_Id,0);
			T=GetToken();
			if (T.Rpar()) T=GetToken();
			else Error(E_Rpar,0);
			if(T.UserId())
			{	Sid=T.GetId();
			...
			if(T.Func()){T=Funktion(C_Pointer,Q);}
			else
			if(T.Array()) {T=ArrayDecl(C_Pointer,Q);}
			else Error(E_ResId,0);
			if (T.Semi()) {} else {T=GetToken();}
		}
		else
		{	Token S;
			int Sid,Tid,Typ;
			...
			{	CreateDecl(Bid,Sid,Typ,K_Simple,S);
				...
				{	S=GetToken();
					Sid=S.GetId();
					if(S.UserId()) {CreateDecl(Bid,Sid,Typ,K_Simple,S);}
					else {Error(E_ResId,0);}
					T=GetToken();
				}
			}
			else
			{	if(S.Func()) T=Funktion(Typ,0);
				else
				if(S.Array())
				{	T=ArrayDecl(Typ,0);
					while(T.Comma()){T=ArrayDecl(Typ,0);}
				}
				else Error(E_ResId,0);
			}
		}
...
****************************************************************** */
	...
	{	int Sid,NofPar,BNr;
		...
		Sid=T.GetId();
		BNr=CreateBlock();
		if (Ty!=C_Void) {CreateDecl(Bid,Sid,Ty,K_Funcval,T);}
		T=GetToken();
		if (T.Lpar())
		{	NofPar=Parameters();
		...
		else {NofPar=0; Error(E_FunHead,0);}
		if (T.Is())
		{	String N;
			T=GetToken();
			if (T.External())
			...
			else {Error(E_Ext,0);return null;}
		}
		else
		{	T=Kropp(T);
			CloseBlock(null);
...
****************************************************************** */
	...
	{	Token T;
		...
		if(!T.Rpar()) {Error(E_Rpar,0);}
		T=GetToken();
		if(!T.Semi()){Error(E_Sem,0);}
		return Num;
	}
...
****************************************************************** */
	...
	{	int M;
		...
		C=new Count();
		T=GetToken();
		while (T.Spec()){T=Spec(GetType(T.GetId()),C);}
		M=C.GetCount();
		if(M!=N)
		{	if (M<N) {Error(E_MissSpec,0);}
			else {Error(E_OverSpec,0);}
		}
		return T;
	}
	...
	{	Token T;
		...
		{	int Q,Sid;
			T=GetToken();
			if(!T.Lpar()) {Error(E_Lpar,0);} else {T=GetToken();}
			if(T.UserId()) {Q=T.GetId();} else {Q=0; Error(E_Id,0);}
			T=GetToken();
			if(T.Rpar()) {T=GetToken();} else {Error(E_Rpar,0);}
			if(T.UserId())
			{	Sid=T.GetId();
		...
		if(T.Semi()) {return GetToken();} else {return T;}
	}
	
	class Count
	{	int Cnt;
		Count(){Cnt=0;}
		void Increment(){Cnt=Cnt+1;}
...
****************************************************************** */
	...
	{	Token T;
		...
		T=GetToken();
		if(!T.Lpar()){Error(E_Lpar,0);}else{T=Alimits(Sid,Qual,OK);}
		if(!T.Rpar()){Error(E_Rpar,0);}else{T=GetToken();}
		return T;
	}
	...
	{	Token T,S;
		...
		{	H=Integer.MAX_VALUE;
			...
			{	CreateExprCall();
				...
				PutCode(Bid,S);
				T=GetToken();
				while(!T.Comma()||T.Rpar()){PutCode(Bid,T);T=GetToken();}
				PutSemi(Bid);
			}
			if(OK) {SetArrayLimit(Sid,Ind,L,H);}
			if(T.Rpar()){Klar=true;}else{Ind=Ind+1;}
		}
...
/* ****************** Interface ********************************** */	
	...
	private Token GetToken() {;return lexanalyzer.GetToken();}
	
	private int CreateBlock(){return Resultat.CreateBlock();}
	private void CloseBlock(Token T)
	{	if(T==null){}
		else
		if(T.GetType()==T_Ignore){}
		else {Resultat.PutCode(T);}
		Resultat.CloseBlock();
	}
	...
	{	boolean OK;
		...
		if(OK){return true;}
		else{Error(E_MultDec,0); return false;}
	}
	private void CreateFuncDecl
		(int Bid,int Sid,int Ty,int K,int NofPar,int B,int Qual,Token Namn)
	{	boolean OK;
		OK=Resultat.CreateDecl(Sid,Ty,K,Namn);
		if(OK){Resultat.PutInf(Sid,NofPar,B,Qual);}
		else{Error(E_MultDec,0);}
	}
	private void Specify(int Sid,int Ty,int K){Resultat.Specify(Sid,Ty,K);}
	...
	private void PutCode(int B, Token T) {Resultat.PutCode(T);}
	...
	private void PutExternal(int B,String N){Resultat.PutExternal(B,N);}
	...
	{	if(I==Id_Integer){return C_Integer;}
		else
		if(I==Id_Real){return C_Real;}
		else
		if(I==Id_Text){return C_Text;}
		else
		if(I==Id_Pointer){return C_Pointer;}
		else{return C_Any;}
	}
