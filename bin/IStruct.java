...
****************************************** */
	...
	*********************** */
		...
		{	DeclNode Nxt;
			Symbol Info;
			void SetInfo(Symbol S){Info=S;}
			Symbol GetSym(){return Info;}
			DeclNode Insert(DeclNode P)
			{	Nxt=P;
				return P;
			}
			DeclNode Search(int Id)
			{	if(Info.Equal(Id)) {return this;}
				else
				if(Nxt==null){return null;}
				else {return Nxt.Search(Id);}
			}
			Symbol Par(int N)
			{	if(Info.GetKind()==K_Funcval){return Nxt.Par(N);}
				else
				if(N==1){return Info;}
				else
				if(Nxt!=null){return Nxt.Par(N-1);}
				else {return null;}
			}
			DeclNode GetSlagSym(int K)
			{	if(Info.GetKind()==K){return this;}
				else
				if(Nxt!=null){return Nxt.GetSlagSym(K);}
				else {return null;}
			}
			...
			{	if(Info!=null){Info.SetLim(Ind,Low,High);}else{}
			}
			void PutString(String T)
			{	if(Info!=null){Info.SetString(T);}else{}
			}
		}
	/* ***********************
	...
	*********************** */
		...
		{	CodeNode Nxt;
			...
			CodeNode P2Init() {Cur=0; return this;}
			CodeNode Reset(int b,int i)
			{	if(b==0){Cur=i; return this;}
				else
				if(Nxt==null){return null;}
				else{return Nxt.Reset(b-1,i);}
			}
			...
			{	Token T;
				T=Code[Cur];
				Cur=Cur+1; TokNr=TokNr+1;
				if (Cur==Max) {CurrentCode=Nxt.P2Init();}
				if(T!=null&&T.Line())
					{Line=T.GetLine(); return CurrentCode.Get();}
				else {return T;}
			}
			int GetCur(){return Cur;}
			void SetCur(int I){Cur=I;}
			void OutPut(outfile Fil)
			{for(int I=0; I<Max; I=I+1){if(Code[I]!=null){Code[I].OutPut(Fil);}}}
		}
	...
	* 
	*********************** */
		Block Insert(Block P) {Nxt=P; return P;}
		
		Block Search(int Bid)
		{	if(Bid==BlockNr){return this;}
			else
				if(Nxt!=null){return Nxt.Search(Bid);}
				else{return null;}
		}
		void SetSF(Block P)
		{	if(P==null){SFBlockNr=-1;}
			else{SFBlockNr=P.GetBn();}
		}
		int GetSFBn(){return SFBlockNr;}
		Block GetSF(){return SFBlock;}
		int GetBn(){return BlockNr;}
		Symbol DeclSearch(int Sid)
		{	DeclNode P;
			if(SymTab!=null){P=SymTab.Search(Sid);}
			else{P=null;}
			if(P!=null){return P.GetSym();}
			else{return null;}
		}
		Symbol GetSlagSym(int Knd)
		{	DeclNode P;
			if(Knd!=LastKind){LastDN=SymTab; LastKind=Knd;}
			if(LastDN!=null){P=LastDN.GetSlagSym(Knd);}
			else{P=null;}
			if(P==null)
			{	Knd=0;
				LastDN=SymTab;
		...
		Token GetCode(){return CurrentCode.Get();}
		int GetCurTokNr(){return TokNr;}

		Symbol GetPar(int N){return SymTab.Par(N);}
		int GetSize(){return NextAdr;}
		/*		*** Help Routines *** */
		...
		{	DeclNode P; CodeNode Q;
			...
			Fil.WriteLine();
			P=SymTab;
			while(P!=null){P.GetSym().OutPut(Fil);P=P.Nxt;}
			Fil.Write("#KOD#");
			Fil.WriteLine();
			Q=BlockCode;
			while(Q!=null){Q.OutPut(Fil); Q=Q.Nxt;}
			Fil.Write("##BLOCKSLUT##");
			Fil.WriteLine();
		}
	}
...
/* ******************** Pass 1 methods ************************* */
	...
	{
		if(CurrentBlock!=null){CurrentBlock.PutCode(Tk);}
	}
	/*-------------------------------------------
	!	CloseBlock
...
*************************************************** */
   ...
   {	
      if(Blocks!=null){CurrentBlock=Blocks.Search(N);}
      else {CurrentBlock=null;}
      if(CurrentBlock!=null){CurrentBlock.InitCode();}
      CurBlkNum=N;
      return N;
   }
/* ***************************************************
...
*************************************************** */
   public int ContBlock(int N,int nr)
	...
	public void Jump(int nr){CurrentBlock.ResetCode(nr);}
/* ***************************************************
...
*************************************************** */
	...
	public int GetNextTokNr(){return CurrentBlock.GetCurTokNr();}
/* ***************************************************
...
*************************************************** */
	...
	{
		Block Q;
		if(Blocks!=null) {Q=Blocks.Search(BId);} else {Q=null;}
		if(Q!=null) {return Q.DeclSearch(SId);} else {return null;}
	}
	public int GetSF(int Bid)
	{	Block Q;
		if(Blocks!=null){Q=Blocks.Search(Bid);} else {Q=null;}
		if(Q!=null){return Q.GetSFBn();}else{return -1;}
	}
	...
	{	Block Q;
		Q=Blocks.Search(Bid);
		if(Q!=null){return Q.GetPar(N);}else{return null;}
	}
	...
	{	Block P;
		...
		Fil.WriteLine();
		P=Blocks;
		while(P!=null){P.OutPut(Fil); P=P.Nxt;}
