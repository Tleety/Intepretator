...
****************************************************************** */
	...
	{	String Rad;
		...
		boolean TomRad;
		char LastCh;
		Inmatning(Filhantering F) {TomRad=true;Fil=F;}
		...
		char GetChar()
		{	char Ch;
			if(Rad.equals("\025")){Ch='\025';}
			else
			if(TomRad){Ch=NL;}
			else
			{	Ch=Rad.charAt(Pos);
				Pos=Pos+1;
				if(Pos==RadSz){TomRad=true;}
			}
			LastCh=Ch;
			return Ch;
		}
		...
		{	String S;
			if(Backa)
			{	if(LastCh==NL){TomRad=true;}
				else {Pos=Pos-1; TomRad=false;}
...
****************************************************************** */
	...
	{	if(Character.isLetter(C)) {return Q1(Inm.GetChar());}
		else
		if(Character.isDigit(C)) {return Q2(Inm.GetChar());}
		else
		if (C=='"') {return Q4(Inm.GetChar());}
		else
		if (Simple(C)) {return CreateToken(T_Op,SimpCode(C),Inm.GetItem(false));}
		else
		if (C==':') {return Q5(Inm.GetChar());}
		else
		if (C=='<') {return Q6(Inm.GetChar());}
		else
		if (C=='>') {return Q7(Inm.GetChar());}
		else
		if (Space(C)) {return Q0(Inm.FirstChar());}
		else
		if (C=='!') {return Q8(Inm.GetChar());}
		else
		if (C==EOF) {return CreateToken(T_Ignore,0,"EOF");}
		else {return ErrorToken(0,"Fel");}
	}
	private Token Q1(char C)
	{	if (Character.isLetterOrDigit(C)) {return Q1(Inm.GetChar());}
		else
		{	String Str; int K;
	...
	{	if (Character.isDigit(C)) {return Q3(Inm.GetChar());}
		else
		{	String Str; 
			Str=Inm.GetItem(true);
	...
	{	if (C=='"') {return CreateToken(T_Text,0,Inm.GetItem(false));}
		else
		if (C==NL) {return ErrorToken(0,"");}
		else {return Q4(Inm.GetChar());}
	}
	private Token Q5(char C)
	{	if (C=='=') {return CreateToken(T_Op,TC_Become,":=");}
		else {return CreateToken(T_Op,4,Inm.GetItem(true));}
	}
	private Token Q6(char C)
	{	if (C=='>') {return CreateToken(T_Op,TC_Neq,"<>");}
		else
		if (C=='=') {return CreateToken(T_Op,TC_Le,"<=");}
		else {return CreateToken(T_Op,TC_Less,Inm.GetItem(true));}
	}
	private Token Q7(char C)
	{	if (C=='=') {return CreateToken(T_Op,TC_Ge,">=");}
		else {return CreateToken(T_Op,TC_Gt,Inm.GetItem(true));}
	}
	private Token Q8(char C)
	{	if (C==';') {return Q0(Inm.GetChar());}
		else
		if (C==NL) {return Q0(Inm.GetChar());}
		else {return Q8(Inm.GetChar());}
	}
	...
	{	if(C=='+') {return TC_Plus;}
		...
		if(C=='-') {return TC_Minus;}
		else
		if(C=='*') {return TC_Mul;}
		else
		if(C=='/') {return TC_Div;}
		else
		if(C=='=') {return TC_Equal;}
		else
		if(C=='(') {return TC_Lpar;}
		else
		if(C==')') {return TC_Rpar;}
